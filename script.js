...
// ============================================
window.obrasData = [];
window.editingId = null;    // ID de la obra que se está editando
window.isLoggedIn = false;  // Estado de sesión
window.currentSlide = 0;    // Slide actual del carrusel del modal
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- 6. FUNCIÓN PARA VALIDAR CAMPOS VACÍOS ---
    ...
    if (artworkForm) {
        artworkForm.addEventListener('submit', async (e) => {
            ...
            campos.forEach(campo => {
                formData.append(campo, document.getElementById(`dash-${campo}`).value);
            });
        ...
        btnUpdate.addEventListener('click', async (e) => {
            ...
            try {
                ...
                if (response.ok && result.success) {
                    ...
                    if (typeof cargarTablaObras === 'function') cargarTablaObras();
                } else {
    ...
    // --- 11. INICIALIZAR LISTA DE PAÍSES ---
    llenarListaPaises();

    // --- 12. EVENTOS DEL MODAL Y FILTROS ---
...
window.prepararEdicion = function (id) {
    ...
    slots.forEach((slot, index) => {
        ...
        const url = (obra.todas_imagenes && Array.isArray(obra.todas_imagenes)) ? obra.todas_imagenes[index] : (index === 0 ? obra.imagen_url : null);
        if (url) {
    ...
    if (btnSave && btnUpdate) {
        ...
        btnUpdate.style.setProperty('display', 'block', 'important');
    }
};
